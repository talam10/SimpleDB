    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                preAppend();
                // some code goes here
                // calling the helper method UPDATE
                update(tid.getId());
           }
        }
    }
    ...
    */
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                recoveryUndecided = false;
                // some code goes here
                // Read the offset of the last valid checkpoint

                // MODIFIED BY TASNIM: I added some more cases like
                // BEGIN_RECORD, CHECKPOINT_RECORD etc.

                raf.seek(0);
                long lastCheckpointOffset = raf.readLong();

                if (lastCheckpointOffset != NO_CHECKPOINT_ID) {
                    raf.seek(lastCheckpointOffset);
                    int recordType = raf.readInt();
                    if (recordType != CHECKPOINT_RECORD) {
                        throw new IOException();
                    }
                    raf.readLong();
                    for (int i = raf.readInt(); i > 0; i--) {
                        long readID = raf.readLong();
                        long startOffset = raf.readLong();
                        tidToFirstLogRecord.put(readID, startOffset);
                    }
                    raf.readLong();
                }
                ...
                while (true) {
                    try {
                        // get the current offset of the file first
                        long cOffset = raf.getFilePointer();
                        int type = raf.readInt();
                        long tid = raf.readLong();

                        // NO NEED FOR CHECKPOINT_RECORD type check

                        if (type == BEGIN_RECORD) {
                            tidToFirstLogRecord.put(tid, cOffset);
                        } else if (type == UPDATE_RECORD) {
                            Page beforeImage = readPageData(raf);
                            Page afterImage = readPageData(raf);
                            Database.getCatalog().getDatabaseFile(beforeImage.getId().getTableId()).writePage(afterImage);
                        } else if (type == COMMIT_RECORD) {
                            tidToFirstLogRecord.remove(tid);
                        } else if (type == ABORT_RECORD) {
                            long offset = raf.getFilePointer();
                            // CALL HELPER METHOD: UPDATE
                            update(tid);
                            raf.seek(offset);
                            tidToFirstLogRecord.remove(tid);
                        } else {
                ...
                // The last checks for openCrash, OpenCommitOpenCrash etc.
                ArrayList<Long> tidFLR = new ArrayList<>(tidToFirstLogRecord.keySet());
                // I made an interface change for this in TransactionID class
                for (long tid : tidFLR) {
    ...
    public void print() throws IOException {
        ...
        while (raf.getFilePointer() < raf.length()) {
            ...
            System.out.println("Record Type: " + typeName + ", Transaction Id: " + tid);
        }
    }
    ...
    // ADDITIONAL HELPER METHOD: We use these type checks for long tuple ID checking.
    // We call this helper method in both rollback and recover
    // This is an updated rollback version that modularizes our code
    private void update(long tid) throws IOException {
        HashMap<PageId, Page> loserTid = new HashMap<>();
        // Get the offset of the first log record for the specified transaction
        ...
        while (true) {
            try {
                ...
                if (type == CHECKPOINT_RECORD) {
                    ...
                    Page beforeImage = readPageData(raf);
                    Page afterImage = readPageData(raf);
                    if (transactionID == tid && !loserTid.containsKey(afterImage.getId())) {
                        loserTid.put(beforeImage.getId(), beforeImage);
                    }
                } else if (type == BEGIN_RECORD || type == COMMIT_RECORD || type == ABORT_RECORD) {
                    // NO ACTIONS NEEDED FOR THESE
                } else {
                    assert false;
                }
                raf.readLong();
        ...
        for (Map.Entry<PageId, Page> PIDEntrySet : loserTid.entrySet()) {
            Page page = PIDEntrySet.getValue();
            Database.getBufferPool().discardPage(PIDEntrySet.getKey());
            Database.getCatalog().getDatabaseFile(PIDEntrySet.getKey().getTableId()).writePage(page);
        }
    }
